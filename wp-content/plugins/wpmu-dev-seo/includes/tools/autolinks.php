...
class Smartcrawl_Autolinks extends Smartcrawl_Base_Controller {
	 ...
	 *
	 * @var Smartcrawl_Object_Cache $cache
	 */
	private $cache;
	...
	protected function __construct() {
		...
		// Cache instance.
		$this->cache = Smartcrawl_Object_Cache::get();
	}
	...
	public function initialize_autolinks() {
		...
		// Get settings data.
		$this->settings = Smartcrawl_Settings::get_options();
		...
		if ( ! is_admin() ) {
			...
			if ( $this->is_option_enabled( 'comment' ) ) {
	...
	private function content_filter( $text, $type = 'post' ) {
		...
		if ( ! $this->is_option_enabled( get_post_type() ) ) {
			return $text;
		}
	...
	private function process_text( $text ) {
		...
		$text = $this->link_taxonomies( $text, $urls, $max_links, $max_single, $max_single_url, $ignored_array );
	...
	private function link_posts( $text, &$urls, &$links, $max_links, $max_single, $max_single_url, $ignored_array, $current_title ) {
		...
		foreach ( $posts as $postitem ) {
			...
			if (
				$this->is_option_enabled( "l{$postitem->post_type}" ) &&
				( ! $max_links || ( $links < $max_links ) ) &&
	 ...
	 * @return string
	 */
	private function link_taxonomies( $text, &$urls, $max_links, $max_single, $max_single_url, $ignored_array ) {
		$links = 0;
		...
		if ( ! empty( $terms ) ) {
			foreach ( $terms as $term ) {
				if ( ( ! $max_links || ( $links < $max_links ) ) && ! in_array( $this->is_option_enabled( 'casesens' ) ? $term->name : strtolower( $term->name ), $ignored_array, true ) ) {
	...
	private function get_enabled_post_types() {
		...
		foreach ( $post_types as $post_type ) {
			...
			if ( $this->is_option_enabled( "l$post_type" ) ) {
	...
	private function get_enabled_taxonomies() {
		...
		foreach ( $taxonomies as $taxonomy ) {
			...
			// Include only if enabled in settings.
			if ( $this->is_option_enabled( "l{$key}" ) ) {
