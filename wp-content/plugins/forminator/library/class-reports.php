...
class Forminator_Reports {
	...
	public function process_report() {
		...
		foreach ( $report_data as $report ) {
			...
			$current_time    = date_i18n( 'Y-m-d H:i', time() );
			// check the next sent.
			$next_sent = null;
			...
			if ( strtotime( $current_time ) > strtotime( trim( $next_sent ) ) ) {
				$mail_sent = $this->send_email_report( $report_value );
				if ( $mail_sent ) {
	...
	public function forminator_email_report_data( $settings ) {
		...
		if ( 'selected' === $module_type ) {
			...
			$module_ids = array_map( function ( $ar ) {
				return $ar->id;
			}, $modules );
		}
		if ( ! empty( $module_ids ) ) {
			foreach ( $module_ids as $m => $module_id ) {
				...
				if ( Forminator_Form_Entry_Model::has_live_payment( $module_id ) ) {
					...
					if ( ! empty( $payment_data ) ) {
						$payment_value = array_map( function ( $payment ) {
